...
def modulador_n(bits, fc, mpp):
    ...
    # Se reacomoda el tamaño del array y se concatena para obtener las parejas de bits
    c = np.array(bits).reshape(105732, 4).tolist()
    ...
    # 4. Asignación de las formas de onda según los bits (16QAM)
    #for element in newListD:
    for i, element in enumerate(newListD):
        if element == '00':
            senal_Tx_I[i*mpp : (i+1)*mpp] = portadoraI*-3
    ...
    for i, element in enumerate(newListE):
        if element == '00':
            senal_Tx_Q[i*mpp : (i+1)*mpp] = portadoraQ*3
...
print('Longitud de los bits después: ', len(bits_Rx))
